...
function Responsibility({responsibility, position, setResponsibility, removeResponsibity}) {
    ...
    useEffect(() => {
        if (focused) {
            const textarea = document.getElementById(`responsibility-${position}`)
    ...
    function handleChange (e) {
        ...
        setResponsibility(textarea.value.replace(/ {2,}|\r\n|\n|\r/gm, ' '))
        adjustHeight(textarea)
    }
            ...
            <div className={`${focused ? 'flex-grow' : ''}`}>
                {
                    focused ? (
                        <textarea
                            ...
                            className="ml-2 block w-full outline-none transition bg-transparent"
                            style={{msOverflowStyle: 'none',scrollbarWidth: 'none',overflowY: 'scroll'}}
                        />
                    ) : (
